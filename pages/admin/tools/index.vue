<template>
  <v-content>
    ...
    <v-data-table v-model="selected" selected-key="id" select-all :headers="headers" :items="tools" :search="search" hide-actions :custom-filter="customSearch">
      <template slot="items" scope="props">
        <td>
          <v-checkbox primary hide-details v-model="props.selected"></v-checkbox>
        </td>
        <td @click.stop="$router.push({ name: 'admin-tools-id', params: { id: props.item.id } })" class="text-xs-left select">{{ props.item.name }}</td>        
        <td @click.stop="$router.push({ name: 'admin-tools-id', params: { id: props.item.id } })" class="text-xs-left select">{{ props.item.description }}</td>        
        <td @click.stop="$router.push({ name: 'admin-tools-id', params: { id: props.item.id } })" class="text-xs-left select">{{ props.item.updatedAt | date }}</td>
    ...
    <v-dialog v-model="dialog" persistent max-width="500">
      <v-form v-model="valid" ref="form" @submit.prevent>
        <v-card>
          <v-card-title>
            <div class="headline">Create new Tool</div>
          </v-card-title>
          <v-card-text>
            <v-text-field ref="name" :rules="nameRules" required v-model="name" name="name" label="Name" id="name"></v-text-field>
            <v-text-field v-model="description" name="description" label="Description" id="description"></v-text-field>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    save () {
      if (this.valid) {
        this.$store.dispatch('tools/create', {
          name: this.name,
    ...
    deleteTools () {
      ...
      this.selected.forEach(item => {
        promises.push(this.$store.dispatch('tools/remove', item.id))
      })
